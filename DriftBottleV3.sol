...
/*
版本2比版本1做的改动如下
1. 将漂流瓶增加一个自增的bottelId,并作为漂流瓶mapping的key, value 为bottle结构.同时将漂流瓶的打开状态用另一个mapping存储
   bottleid=> bool
2. 取漂流瓶是用户根据对应的bottleId来取,并进行判断当前id是否已经打开,如果已经打开,则直接require回滚.如果这个漂流瓶状态是未打开
   则取出数据返回即可
3. 增加根据bottleId查询bottle状态的方法.
*/
contract DriftBottleV3 {
    //当前用户数据mapping定义为私有
    ...
    */
    event CatchBottle(address indexed userId, uint64 bottleId, uint256 amount);
    ...
    */
        ...
        returns (string memory _greetings, string memory _wechat)
    {
        ...
        emit CatchBottle(msg.sender, _bottleId, amount);
